...
These errors are specific to AI streaming operations.

### `ActorConnectionError`

Thrown when connecting to the streaming actor fails.

```typescript
class ActorConnectionError {
	readonly _tag = "ActorConnectionError"
	readonly message: string
	readonly cause: unknown
}
```
...
Thrown when a streaming actor operation fails.

```typescript
class ActorOperationError {
	readonly _tag = "ActorOperationError"
	readonly message: string
...
Thrown when creating a streaming message fails.

```typescript
class MessageCreateError {
	readonly _tag = "MessageCreateError"
	readonly message: string
...
Thrown when processing an AI content chunk fails.

```typescript
class StreamProcessingError {
	readonly _tag = "StreamProcessingError"
	readonly message: string
...
Thrown when persisting the final streamed message fails.

```typescript
class MessagePersistError {
	readonly _tag = "MessagePersistError"
	readonly message: string
...
Thrown when the bot is not properly configured for streaming.

```typescript
class BotNotConfiguredError {
	readonly _tag = "BotNotConfiguredError"
	readonly message: string
