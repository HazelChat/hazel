 ...
 */
const oauthRetrySchedule = Schedule.exponential("100 millis").pipe(
 ...
 */
const handleGetOAuthUrl = Effect.fn("integrations.getOAuthUrl")(
	function* (path: {
...
const handleOAuthCallback = Effect.fn("integrations.oauthCallback")(function* (
	...
	if (!encodedState && installation_id && setup_action === "update") {
		...
		if (Option.isNone(connectionOption)) {
			...
			yield* Effect.logWarning(
				"GitHub update callback for unknown installation",
				{
		...
		if (Option.isNone(orgOption)) {
			yield* Effect.logWarning(
				"GitHub update callback: organization not found",
				{
	...
	const parsedState = yield* Effect.try({
		try: () =>
			Schema.decodeUnknownSync(OAuthState)(
	...
	const redirectWithError = (errorCode: OAuthErrorCode) =>
		HttpServerResponse.redirect(
			buildRedirectUrl(parsedState.returnTo, provider, "error", errorCode),
		);
	...
	if (isProduction && parsedState.environment === "local") {
		...
		const localUrl = new URL(
			`http://localhost:3003/integrations/${provider}/callback`,
		);
		if (installation_id)
			localUrl.searchParams.set("installation_id", installation_id);
	...
	const tokensResult = yield* oauthProvider
	...
	const accountInfoResult = yield* oauthProvider
		.getAccountInfo(tokens.accessToken)
		.pipe(
			Effect.retry({
				schedule: oauthRetrySchedule,
				while: isRetryableError,
	...
	yield* IntegrationBotService.addBotToOrg(
		provider,
 ...
 */
const handleGetConnectionStatus = Effect.fn("integrations.getConnectionStatus")(
	function* (path: {
		orgId: OrganizationId;
		provider: IntegrationConnection.IntegrationProvider;
	}) {
...
export const HttpIntegrationLive = HttpApiBuilder.group(
	HazelApi,
	"integrations",
	(handlers) =>
		handlers
