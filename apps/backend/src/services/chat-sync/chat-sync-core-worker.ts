...
export const ChatSyncCoreWorkerMake = Effect.gen(function* () {
	...
	const translateHazelMentionsForDiscord = Effect.fn("discordSyncWorker.translateHazelMentionsForDiscord")(
				...
				client
					...
					.innerJoin(
						...
						and(
							...
							eq(schema.chatSyncChannelLinksTable.syncConnectionId, params.syncConnectionId),
	...
	const resolveHazelMessageId = Effect.fn("discordSyncWorker.resolveHazelMessageId")(function* (params: {
		...
		const links = yield* db.execute((client) =>
			client
				...
				.where(
					and(
						eq(schema.chatSyncMessageLinksTable.externalMessageId, params.externalMessageId),
						isNull(schema.chatSyncMessageLinksTable.deletedAt),
					),
				)
				.limit(1),
	...
	const resolveOrCreateOutboundLinkForMessage = Effect.fn(
		...
		const rootMessageLinkOption = yield* messageLinkRepo.findByHazelMessage(parentLink.id, rootMessageId)
	...
	const syncHazelMessageToProvider = Effect.fn("discordSyncWorker.syncHazelMessageToProvider")(function* (
		syncConnectionId: SyncConnectionId,
		hazelMessageId: MessageId,
	...
	const syncConnection = Effect.fn("discordSyncWorker.syncConnection")(function* (
		...
		for (const link of links) {
			...
			for (const unsyncedMessage of unsyncedMessages) {
				const result = yield* syncHazelMessageToProvider(syncConnectionId, unsyncedMessage.id).pipe(
					Effect.result,
				)
	...
	const syncHazelMessageUpdateToProvider = Effect.fn("discordSyncWorker.syncHazelMessageUpdateToProvider")(
		function* (
			syncConnectionId: SyncConnectionId,
			hazelMessageId: MessageId,
			dedupeKeyOverride?: string,
	...
	const syncHazelMessageDeleteToProvider = Effect.fn("discordSyncWorker.syncHazelMessageDeleteToProvider")(
		function* (
			syncConnectionId: SyncConnectionId,
			hazelMessageId: MessageId,
			dedupeKeyOverride?: string,
	...
	const syncHazelReactionDeleteToProvider = Effect.fn(
		...
		const messageLinkOption = yield* messageLinkRepo.findByHazelMessage(link.id, payload.hazelMessageId)
		...
		const targets = yield* db.execute((client) =>
			client
				...
				.innerJoin(
					...
					eq(schema.chatSyncConnectionsTable.id, schema.chatSyncChannelLinksTable.syncConnectionId),
				)
				.where(
					and(
