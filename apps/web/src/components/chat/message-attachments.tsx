...
function ImageAttachmentItem({
	...
	const publicUrl =
		import.meta.env.VITE_R2_PUBLIC_URL || "https://cdn.hazel.sh";
...
export function MessageAttachments({ messageId }: MessageAttachmentsProps) {
	...
	const otherFiles = attachments.filter((attachment) => {
		const fileType = getFileTypeFromName(attachment.fileName);
		return !["jpg", "png", "gif", "webp", "svg", "mp4", "webm"].includes(
			fileType,
		);
	});
			...
			{images.length > 0 && (
				...
				>
					{images.slice(0, 4).map((attachment, index) => (
						<div
							key={attachment.id}
							className={getImageWrapperClass(images.length, index)}
						>
							<ImageAttachmentItem
