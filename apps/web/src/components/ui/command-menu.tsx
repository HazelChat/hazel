      ...
      <ModalContext value={{ isOpen: props.isOpen, onOpenChange: onOpenChange }}>
        ...
        >
          <Modal
            className={({ isExiting, isEntering }) =>
              twMerge(
                "row-start-2 bg-primary text-primary ring-primary text-left shadow-lg outline-none ring md:row-start-1",
    ...
    <CollectionRendererContext.Provider value={renderer}>
      <MenuPrimitive
        className={composeTailwindRenderProps(
          className,
          "grid max-h-full flex-1 grid-cols-[auto_1fr] content-start overflow-y-auto border-t border-primary p-2 sm:max-h-110 *:[[role=group]]:mb-6 *:[[role=group]]:last:mb-0",
        )}
        {...props}
      />
...
const CommandMenuDescription = ({ className, ...props }: TextProps) => {
  ...
  <Text
    slot="description"
    className={twMerge("col-start-3 row-start-1 ml-auto font-normal text-muted-fg text-sm", className)}
    {...props}
  />
  )
}
...
const CommandMenuLabel = ({ className, ...props }: TextProps) => {
  return (
    <Text slot="label" className={twMerge("col-start-2", className)} {...props} />
  )
}
