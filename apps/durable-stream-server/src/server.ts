...
import { Duration, Effect, Layer, Option, Redacted, Stream } from "effect"

import { StreamStoreService, normalizeContentType } from "./StreamStore.ts"
import { DEFAULT_CURSOR_EPOCH, DEFAULT_CURSOR_INTERVAL_SECONDS, generateResponseCursor } from "./Cursor.ts"
import { Headers as ProtocolHeaders } from "./types.ts"
import { StreamAuth, type AuthContext } from "./services/StreamAuth.ts"
import type { HttpServerError } from "@effect/platform"
import type { StreamNotFoundError } from "./errors.ts"
import type { ServerConfigShape, AuthConfigShape } from "./Config.ts"
...
function addCorsHeaders(
	...
	return response.pipe(
		...
		HttpServerResponse.setHeader(`access-control-allow-methods`, `GET, POST, PUT, DELETE, HEAD, OPTIONS`),
 ...
 */
type AuthResult =
	| { authorized: true; auth: AuthContext }
	| { authorized: false; response: HttpServerResponse.HttpServerResponse }

/**
 * Create auth middleware for read operations.
 * Returns either the auth context or an error response.
 */
const authenticateAndAuthorizeRead = (
	authEnabled: boolean,
): Effect.Effect<AuthResult, never, HttpServerRequest.HttpServerRequest | StreamAuth | PgClient.PgClient> =>
	Effect.gen(function* () {
		if (!authEnabled) {
			// Auth disabled - allow all
			return { authorized: true, auth: { type: "service" } as AuthContext }
		}
		...
		// Skip auth for non-bot paths (allows legacy behavior)
		if (!path.startsWith(`/bots/`)) {
			return { authorized: true, auth: { type: "service" } as AuthContext }
		}

		const authHeader = getHeader(request.headers, `authorization`)
		const streamAuth = yield* StreamAuth

		// Authenticate
		const authResult = yield* streamAuth.authenticate(authHeader).pipe(
			Effect.map((auth): AuthResult => ({ authorized: true, auth })),
			Effect.catchTags({
				AuthenticationRequiredError: (err) =>
					Effect.succeed<AuthResult>({
						authorized: false,
						response: addCorsHeaders(createResponse(err.message, { status: 401 })),
					}),
		...
		const authzResult = yield* streamAuth.authorizeRead(authResult.auth, path).pipe(
			Effect.map((): AuthResult => ({ authorized: true, auth: authResult.auth })),
			Effect.catchTag("AccessDeniedError", (err) =>
				Effect.succeed<AuthResult>({
					authorized: false,
					response: addCorsHeaders(createResponse(err.message, { status: 403 })),
				}),
			),
		)
...
/**
 * Create auth middleware for write operations.
 * Returns either the auth context or an error response.
 */
const authenticateAndAuthorizeWrite = (
	authEnabled: boolean,
): Effect.Effect<AuthResult, never, HttpServerRequest.HttpServerRequest | StreamAuth | PgClient.PgClient> =>
	Effect.gen(function* () {
		if (!authEnabled) {
		...
		const authzResult = yield* streamAuth.authorizeWrite(authResult.auth, path).pipe(
			Effect.map((): AuthResult => ({ authorized: true, auth: authResult.auth })),
			Effect.catchTag("AccessDeniedError", (err) =>
				Effect.succeed<AuthResult>({
					authorized: false,
 ...
 */
const makeRouter = (port: number, host: string, config: ServerConfigShape, authConfig: AuthConfigShape) =>
	Effect.gen(function* () {
		 ...
		 */
		const handleOptions = Effect.succeed(addCorsHeaders(HttpServerResponse.empty({ status: 204 })))
		...
		const handleCreate = Effect.gen(function* () {
			...
			const authResult = yield* authenticateAndAuthorizeWrite(authEnabled)
			if (!authResult.authorized) {
			...
			yield* Effect.logInfo(`Handling PUT request`).pipe(Effect.annotateLogs({ path }))
			...
			if (ttlHeader && expiresAtHeader) {
				return addCorsHeaders(
					createResponse(`Cannot specify both Stream-TTL and Stream-Expires-At`, { status: 400 }),
				)
			}
			...
			return yield* store
				...
				.pipe(
					...
					Effect.catchTag(`StreamConflictError`, (err) =>
						Effect.succeed(addCorsHeaders(createResponse(err.message, { status: 409 }))),
					),
				)
		...
		const handleHead = Effect.gen(function* () {
			// Auth check for read operation
			const authResult = yield* authenticateAndAuthorizeRead(authEnabled)
			if (!authResult.authorized) {
				return authResult.response
			}
			...
			return yield* store.get(path).pipe(
				...
				Effect.catchTag(`StreamNotFoundError`, () =>
					Effect.succeed(addCorsHeaders(createResponse(`Stream not found`, { status: 404 }))),
				),
			)
		...
		const createSSEStream = (
			...
			Stream.unfoldEffect({ phase: `read`, offset: initialOffset } as SSEState, (state) =>
				Effect.gen(function* () {
					if (state.phase === `read`) {
						...
						const controlOffset =
							readResult.messages[readResult.messages.length - 1]?.offset ?? streamCurrentOffset
						...
						return Option.some([``, { phase: `read` as SSEPhase, offset: state.offset }] as const)
					}
				}),
		...
		const handleRead = Effect.gen(function* () {
			...
			yield* Effect.logInfo(`Handling GET request`).pipe(Effect.annotateLogs({ path }))
				...
				Effect.flatMap((stream) =>
					Effect.gen(function* () {
						...
						if (liveMode === `sse`) {
							...
							const sseByteStream = sseStream.pipe(Stream.map((s) => encoder.encode(s)))
		...
		const handleAppend = Effect.gen(function* () {
			...
			yield* Effect.logInfo(`Handling POST request`).pipe(Effect.annotateLogs({ path }))
			...
			if (hasProducerId && producerId !== ``) {
				...
				if (epochNum === null) {
					return addCorsHeaders(
						createResponse(`Invalid Producer-Epoch: must be a non-negative integer`, {
							status: 400,
						}),
					)
				}
				...
				if (seqNum === null) {
					return addCorsHeaders(
						createResponse(`Invalid Producer-Seq: must be a non-negative integer`, {
							status: 400,
						}),
					)
				}
			...
			return yield* store
				...
				.pipe(
					...
					Effect.catchTags({
						...
						InvalidJsonError: () =>
							Effect.succeed(addCorsHeaders(createResponse(`Invalid JSON`, { status: 400 }))),
						EmptyArrayError: () =>
							Effect.succeed(
								addCorsHeaders(
		...
		const handleDelete = Effect.gen(function* () {
			...
			yield* Effect.logInfo(`Handling DELETE request`).pipe(Effect.annotateLogs({ path }))
...
/**
 * Default auth configuration (auth disabled).
 */
const defaultAuthConfig: AuthConfigShape = {
	databaseUrl: Redacted.make(``),
	serviceToken: Redacted.make(``),
	authEnabled: false,
}
...
export const makeServerLayer = (
	...
	authConfig: AuthConfigShape = defaultAuthConfig,
): Layer.Layer<never, HttpServerError.ServeError, StreamStoreService | StreamAuth | PgClient.PgClient> =>
	Layer.unwrapEffect(
		Effect.gen(function* () {
			const router = yield* makeRouter(port, host, config, authConfig)
			...
			return HttpServer.serve(router).pipe(HttpServer.withLogAddress, Layer.provide(ServerLive))
		}),
	)
...
export const runServer = (
	...
	config: ServerConfigShape = defaultConfig,
	authConfig: AuthConfigShape = defaultAuthConfig,
): Effect.Effect<void, HttpServerError.ServeError, StreamStoreService | StreamAuth | PgClient.PgClient> =>
	Effect.gen(function* () {
		const router = yield* makeRouter(port, host, config, authConfig)

		yield* Effect.log(`Starting Effect Durable Streams server on http://${host}:${port}`)

		if (authConfig.authEnabled) {
			yield* Effect.log(`Authentication enabled for /bots/* paths`)
		} else {
