 ...
 * @category sensitive
 */
export const Sensitive = <S extends Schema.Schema.All | Schema.PropertySignature.All>(schema: S): Sensitive<S> =>
	Field({
		select: schema,
		insert: schema,
		update: schema,
		...
		return (partitionValue: Schema.Schema.Type<S["fields"][PartitionKey]>) =>
			(params: {
				...
				Effect.gen(function* () {
					...
					const nextCursor =
						hasNext && data.length > 0 ? Option.some(data[data.length - 1][cursorColumn]) : Option.none()

					const previousCursor = params.cursor && hasPrevious ? Option.some(params.cursor) : Option.none()
