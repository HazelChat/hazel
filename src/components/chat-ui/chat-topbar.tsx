...
export function ChatTopbar() {
	...
	return <div class="flex h-16 items-center justify-between gap-2 border-b bg-sidebar p-3">
		<div class="flex items-center gap-2">
			<Show when={isSingleDm()}>
                <Avatar>
                    <AvatarImage src={friends()[0].avatarUrl} alt={friends()[0].displayName} />
                    <AvatarFallback>
                        {friends()[0].displayName.slice(0, 2)}
                    </AvatarFallback>
                </Avatar>
            </Show>
            <Show when={!isSingleDm()}>
            ...
            <p class="text-sidebar-fg">{friends().map((friend) => friend.displayName).join(", ")}</p>
		</div>
...
function Search() {
    ...
    return <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search-icon lucide-search mr-2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
}